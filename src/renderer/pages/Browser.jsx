...
export default function Browser() {
  ...
  return (
    <div
      style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--mub-bg-card)', overflow: 'hidden' }}
      role="application"
      aria-label="浏览器"
    >
      {/* ─── 标签栏 ─── */}
      <div className="mub-tab-bar" role="tablist" aria-label="浏览器标签">
        <Tabs
          type="editable-card"
          size="small"
            ...
            label: (
              <Dropdown menu={{ items: getTabMenuItems(tab.key) }} trigger={['contextMenu']}>
                <span
                  ...
                  aria-label={`${tab.title} - ${tab.containerName || '默认'}`}
                >
                  <span
                    style={{
                  ...
                  {tab.isLoading
                    ...
                    : tab.favicon
                      ? <img src={tab.favicon} alt="" style={{ width: 14, height: 14 }} aria-hidden="true" />
                      : <GlobalOutlined style={{ fontSize: 11, color: '#bbb' }} aria-hidden="true" />
                  }
                  {tab.title}
                </span>
              </Dropdown>
      ...
      {/* ─── 导航栏 ─── */}
      <div className="mub-nav-bar" role="toolbar" aria-label="导航工具栏">
        {/* 导航按钮组 */}
        <div className="mub-nav-buttons">
          <Tooltip title="后退">
            <Button
          ...
          <Tooltip title={activeTab?.isLoading ? '停止' : '刷新'}>
            <Button
              ...
              aria-label={activeTab?.isLoading ? '停止加载' : '刷新页面'}
            />
          </Tooltip>
          <Tooltip title="主页">
            <Button
              size="small" type="text" icon={<HomeOutlined />}
        ...
        >
          <Tooltip title="切换当前标签身份">
            <Tag
              className="mub-container-selector"
              color={activeTab?.containerColor || '#8c8c8c'}
              role="button"
              aria-label={`当前身份: ${activeTab?.containerName || '默认'}`}
              aria-haspopup="listbox"
            >
              <span
                className="mub-container-dot"
                style={{ background: activeTab?.containerColor || '#8c8c8c' }}
                aria-hidden="true"
              />
        ...
        {/* 地址栏 */}
        <div className="mub-address-bar">
          {isSecure
            ? <LockOutlined style={{ color: 'var(--mub-success)', fontSize: 12, marginRight: 6 }} aria-label="安全连接" />
            : <GlobalOutlined style={{ color: 'var(--mub-text-muted)', fontSize: 12, marginRight: 6 }} aria-label="普通连接" />
          }
          <input
            className="mub-address-bar__input"
            value={address}
      ...
      {/* ─── webview 容器 ─── */}
      <div className="mub-webview-container" role="tabpanel">
        {tabs.length === 0 ? (
          ...
          tabs.map((tab) => (
            <div
              key={tab.key}
              className={`mub-webview-wrapper${tab.key === activeTabKey ? '' : ' mub-webview-wrapper--hidden'}`}
              role="tabpanel"
              aria-label={`标签页: ${tab.title}`}
            >
