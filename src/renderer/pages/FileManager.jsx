  ...
  const columns = [
    {
      ...
      width: 40,
      render: (isDir) => (
        <div className={`mub-table-icon${isDir ? ' mub-table-icon--folder' : ''}`} aria-hidden="true">
          {isDir
            ? <FolderOutlined />
            : <FileOutlined />
          }
    ...
    },
    {
      ...
      render: (name, record) => record.isDirectory ? (
        <a
          ...
          role="button"
          aria-label={`进入文件夹 ${name}`}
        >
          {name}
        </a>
      ) : (
        <span style={{ color: 'var(--mub-text)' }}>{name}</span>
      ),
    },
    {
      title: '大小',
      ...
      render: (_, record) => (
        <Space size={4} role="group" aria-label={`${record.name} 操作`}>
          ...
          <Popconfirm title={`确定删除 "${record.name}"？`} onConfirm={() => handleDelete(record.path)}>
            <Tooltip title="删除">
              <Button type="text" size="small" danger icon={<DeleteOutlined />} aria-label={`删除 ${record.name}`} />
            </Tooltip>
          </Popconfirm>
        </Space>
      ),
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        padding: '8px 12px', background: 'var(--mub-bg-card)', borderRadius: 8,
        border: '1px solid var(--mub-border-light)',
      }} role="toolbar" aria-label="路径导航">
        <Tooltip title="根目录">
          <Button
      ...
      <Modal
        ...
        okText="创建"
        cancelText="取消"
        aria-label={createType === 'folder' ? '新建文件夹对话框' : '新建文件对话框'}
      >
